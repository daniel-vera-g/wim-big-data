...
def run_preprocessing_pipeline(combined_artist_df, indexers, encoders, assembler, scaler):
...
def process_and_write_batch(batch_df, batch_id, output_dir):
    ...
    processed_data = run_preprocessing_pipeline(batch_df, transformation_data[0], transformation_data[1], normalized_data[0], normalized_data[1])
...
# for feature in features:
    # Check how much rows are in feature table
...
for offset in range(0, total_rows, fetchsize):
    ...
    process_and_write_batch(combined_artist_df, batch_id, output_dir)
...
for batch_id in range(num_batches):
    ...
    if not os.path.exists(batch_output_path):
        # logger(f"[MAIN_COMBINING]: Batch {batch_id} is missing. Skipping ...")
        continue
